...
module.exports = function LocalUserHandler(server) {
		...
		var p = new Promise(function(resolve, reject) {
			...
			AclUser.findOne(qry, function(err, usr) {
				...
				usr.save(function(err, user) {
					...
					const noReply = server.registry.get('transom-options.api_definition.smtp.noreply');
					assert(noReply && noReply.email, "api_definition.smtp.noreply is not specified!");
						
					const transomSmtp = server.registry.get('transomSmtp');
					const transomTemplate = server.registry.get('transomTemplate');

					transomSmtp.sendMail({
						from: noReply.email,
						fromname: noReply.name || noReply.email,
