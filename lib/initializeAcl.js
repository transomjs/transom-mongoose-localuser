...
module.exports = function AclInitialiser() {
	...
	function createGroups(server) {
		...
		const dbMongoose = server.registry.get('transom-options.api_definition.db_mongoose', null);
		if (dbMongoose) {
			...
			// Attempt to insert each Group. Existing ones will fail on a UQ.
	...
	function createDefaultUser(server) {
		...
		const newUser = new AclUserModel({
			email: server.registry.get('transom-options.administrator_email', 'administrator@localhost.net'),
